...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.redirect(new URL(`/?error=auth-failed&desc=${encodeURIComponent(errorDesc || "no_code")}`, request.url));
  } catch (err: any) {
    console.error("[Auth Callback] Unexpected exception:", err);
    return NextResponse.redirect(new URL("/?error=auth-unexpected", request.url));
  }
}
